...
const headers = ref([
  ...
  { title: 'Event Type Name', key: 'name', align: 'start' },
...
const eventTypes = ref([
  { id: 1, name: 'Seminar', organization: 'VIT, Pune', isActive: true },
  { id: 2, name: 'Workshop', organization: 'VIT, Pune', isActive: true },
  { id: 3, name: 'Conference', organization: 'VIT, Pune', isActive: true },
  { id: 4, name: 'Webinar', organization: 'VIT, Pune', isActive: false },
  { id: 5, name: 'Guest Lecture', organization: 'VIT, Pune', isActive: true },
]);

// --- Computed Properties ---
const isEditing = computed(() => !!editedItem.value.id);
const formTitle = computed(() => (isEditing.value ? 'Edit Event Type' : 'Add Event Type'));

// --- Form Validation Rules ---
const nameRules = [
  (v) => !!v || 'Event Type name is required.',
];
...
const deleteItemConfirm = () => {
  if (itemToDelete) {
    const index = eventTypes.value.findIndex(et => et.id === itemToDelete.id);
    if (index > -1) {
...
const saveEventType = async () => {
  ...
  if (isEditing.value) {
    const index = eventTypes.value.findIndex(et => et.id === editedItem.value.id);
    if (index > -1) {
      Object.assign(eventTypes.value[index], editedItem.value);
    }
  } else {
    const newItem = {
      ...editedItem.value,
    ...
    eventTypes.value.unshift(newItem);
  }
  closeDialog();
};
  ...
  <v-container fluid>
    ...
    <v-dialog v-model="dialog" max-width="500px" persistent>
      <v-card>
        ...
        <v-card-text class="pa-4">
           <v-form ref="form" @submit.prevent="saveEventType">
              <v-text-field
        ...
        <v-card-actions class="pa-4">
          ...
          <v-btn color="primary" variant="flat" @click="saveEventType">
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="deleteDialog" max-width="400px" persistent>
        <v-card>
            ...
            <v-card-text>Are you sure you want to delete this event type?</v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
